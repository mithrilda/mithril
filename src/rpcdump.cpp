// Copyright (c) 2009-2012 Mithril Developers
...
Value importprivkey(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 3)
        throw runtime_error(
            "importprivkey <mithrilprivkey> [label] [rescan=true]\n"
...
Value dumpprivkey(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "dumpprivkey <mithriladdress>\n"
            "Reveals the private key corresponding to <mithriladdress>.");
...
Value dumpwallet(const Array& params, bool fHelp)
{
    ...
    // produce output
    file << strprintf("# Wallet dump created by Mithril %s (%s)\n", CLIENT_BUILD.c_str(), CLIENT_DATE.c_str());
